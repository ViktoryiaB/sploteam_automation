...
public class SploteamTest_Victoria_HW {
    ...
    @Test //HW5
    public void numberOfCampsPerMonthIsDisplayed() { //HW5
        assertTrue(driver.findElement(By.className(CAMP_BUTTON_CLASS)).isDisplayed());
    ...
    public void EditDateOfBirthNegativePersonalAccount() {   //HW5
        ...
        WebElement windowPicker2 = driver.findElements(By.className(CLEAR_FORM_DATEOFBIRTH_CLASS)).get(1);
        String initialwindowPicker = windowPicker2.getAttribute("value");
        windowPicker2.click();
        if (windowPicker2.getAttribute("value").isEmpty()) {
       ...
       // windowPicker2.getAttribute("value");
        ...
        System.out.println(windowPicker2Value);
        System.out.println("Expected: " + initialwindowPicker);
        System.out.println("Observed:" + windowPicker2Value);
        assertEquals(initialwindowPicker,windowPicker2Value);
    ...
    public void assertSportTypeFilter() {
        ...
        List<WebElement> menuItems = driver.findElements(By.className(ARENA_FILER_OPTIONS_CLASS));

        int footballMenuIndex = indexOfMenuItem(menuItems,football);

        menuItems.get(footballMenuIndex).click();

        assertEquals(football, sportsTypeFilter.getText());

        assertFalse(driver.findElements(By.className(ARENA_FILER_CLASS)).isEmpty());
        for (int i = 0; i < driver.findElements(By.className(ARENA_FILER_CLASS)).size(); i++) {
            assertEquals(football, driver.findElements(By.className(ARENA_FILER_CLASS)).get(i).getText());
        }
        ...
        for (int i = 0; i < driver.findElements(By.className(SPORT_TYPE_ON_GAME_CLASS)).size(); i++) {
            assertEquals(football, driver.findElements(By.className(SPORT_TYPE_ON_GAME_CLASS)).get(i).getText());
        }*/
    }
    ...
    public void assertChangeCity() {
        ...
        assertEquals(city, driver.findElement(By.className(CITY_HEADER_CLASS)).getText());
        ...
        wait.until(ExpectedConditions.not(textToBe(By.className(CITY_HEADER_CLASS), city)));
        ...
        wait.until(textToBe(By.className(CITY_HEADER_CLASS), city));
    }
    ...
    public void assertSurveyCityNegative() throws InterruptedException {
        ...
        wait.until (ExpectedConditions.presenceOfElementLocated(By.xpath(SURVEY_OPTION_HEADER_VISIBILITY_XPATH)));
        assertEquals(city, driver.findElement(By.xpath(CITY_CURRENT_SURVEY_XPATH)).getText());
        ...
        wait.until (ExpectedConditions.invisibilityOfElementLocated(By.className(CITY_OPTIONS_SURVEY_CLASS)));
    ...
    public void EditGenderPersonalAccount() {//TS получается что изменение пола просиходит только один раз а второй пол остается не проверенным? давай вместе перепишем на цикл
        ...
        assertNotEquals(currentGenderValue, genderField.getText());
    ...
    int indexOfMenuItem(List<WebElement> menuList, String menuItem) throws NoSuchElementException
    {
        int i = 0;
        for (WebElement tempElement: menuList)
        {
            if (tempElement.getText().equalsIgnoreCase(menuItem)) { return i;}
                else {i++;}
        }
        throw new NoSuchElementException("Item " + menuItem + " not found" );
    }
    ...
    public void assertArenaFilterForEachDate() throws InterruptedException {
        ...
        int demoMenuIndex = indexOfMenuItem(menuItems,"Demo");

        driver.findElements(By.className(ARENA_FILER_OPTIONS_CLASS)).get(demoMenuIndex).click();
        assertEquals("Demo", arenaFilter.getText());
        for (int i = 0; i < driver.findElements(By.className(DATE_PICKER_CLASS)).size(); i++) {
            ...
            if (driver.findElements(By.className(ARENA_ON_GAME_CLASS)).isEmpty()) {
                assertTrue(driver.findElement(By.className(EMPTY_GAMES_ERROR_MESSAGE_CLASS)).isDisplayed());
                assertEquals("Упс... на этот день нет ни одной игры, но Вы можете найти подходящую игру в другой день.", driver.findElement(By.className(EMPTY_GAMES_ERROR_MESSAGE_CLASS)).getText());
            } else {
                ...
                for (int j = 0; j < driver.findElements(By.className(ARENA_ON_GAME_CLASS)).size(); j++) {
                    assertEquals("\"Demo\"", driver.findElements(By.className(ARENA_ON_GAME_CLASS)).get(j).getText());
                }
            }
        }
    }
    ...
    public void assertArenaFilterPlayerLevel() throws InterruptedException {
        driver.findElement(By.xpath(MORE_GAME_BUTTON_XPATH)).click();
        sleep(2000);
        //wait.until(ExpectedConditions.presenceOfElementLocated(By.className(ARENA_FILER_CLASS))); // wait.until does not work here
        ...
        assertEquals("Медиум", arenaFilter.getText());
        for (int i = 0; i < driver.findElements(By.className(DATE_PICKER_CLASS)).size(); i++) {
            driver.findElements(By.className(DATE_PICKER_CLASS)).get(i).click();
            if (driver.findElements(By.className(ARENA_ON_GAME_CLASS)).isEmpty()) {
                assertTrue(driver.findElement(By.className(EMPTY_GAMES_ERROR_MESSAGE_CLASS)).isDisplayed());
    ...
    public void assertChangeCityRefactor() {
        ...
        wait.until(textToBe(By.className(CITY_HEADER_CLASS), onlyCity));
        driver.findElement(By.className(CITY_HEADER_CLASS)).click();
        driver.findElements(By.className(CITY_OPTIONS_CLASS)).get(1).click();
        wait.until(textToBe(By.className(CITY_HEADER_CLASS),city));
    }
...
/*
    ...
    */
    ...
    public static String SURVEY_OPTION_HEADER_VISIBILITY_XPATH="//*[@id=\"root\"]/div[2]/div/div[3]/div[1]/div/div[4]";
